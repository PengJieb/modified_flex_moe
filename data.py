...
def load_ehr_clinic_data(clinic_path):
    # clinic_data = pd.read_csv(clinic_path)
    
    data_list = [pd.read_csv(cp) for cp in clinic_path]
    clinic_data = pd.concat(data_list, ignore_index=True)
    
    value_set = {}
    for kk in clinic_data:
        if kk != 'Patient_ID':
            if type(clinic_data[kk].values[0]) == str:
                value_set[kk] = list(set(clinic_data[kk].values))
    
    ret_data = {}
    feat_dim = 0
    for i in range(len(clinic_data)):
        pid = clinic_data['Patient_ID'][i]
        ...
        for kk in clinic_data:
            ...
            if kk != 'Patient_ID':
                if type(clinic_data[kk].values[0]) == str:
                    data_vector.append(value_set[kk].index(clinic_data[kk][i]))
                else:
                    data_vector.append(clinic_data[kk][i])
        ret_data[pid] = np.array(data_vector)
        feat_dim = ret_data[pid].shape[0]
    return ret_data, feat_dim

def load_ehr_genomic_data(genomic_path):
    # genomic_data = pd.read_csv(genomic_path)
    data_list = [pd.read_csv(gp) for gp in genomic_path]
    genomic_data = pd.concat(data_list, ignore_index=True)
    
    ret_data = {}
    feat_dim = 0
    for i in range(len(genomic_data)):
        pid = genomic_data['Patient_ID'][i]
        
        data_vector = []
        for kk in genomic_data:
            
            if kk != 'Patient_ID':
                data_vector.append(genomic_data[kk][i])
        ret_data[pid] = np.array(data_vector)
        feat_dim = ret_data[pid].shape[0]
    return ret_data, feat_dim

def load_and_preprocess_ehr_data(args, modality_dict):
    clinic_path = ['data/adni/data_clinic.csv']
    genomic_path = ['data/adni/data_gene.csv']
    label_path = ['data/adni/data_label_processed.csv']
    
    label_list = [pd.read_csv(lp) for lp in label_path]
    ...
    data_splits = label_df['split'].values
    ...
    common_idx_list = []
    observed_idx_arr = np.zeros((labels.shape[0], 2), dtype=bool)
    
    modality_combinations = [''] * len(data_splits)
    def update_modality_combinations(idx, modality):
        nonlocal modality_combinations
        if modality_combinations[idx] == '':
            modality_combinations[idx] = modality
    ...
    if 'C' in args.modality or 'c' in args.modality:
        clinic_data, feat_dim = load_ehr_clinic_data(clinic_path)
        tmp = np.zeros((len(id_to_idx), feat_dim))
        filtered_idx = []
        for pid in clinic_data:
            tmp[id_to_idx[pid]] = clinic_data[pid]
            observed_idx_arr[id_to_idx[pid]][modality_dict['clinic']] = True
            filtered_idx.append(id_to_idx[pid])
        data_dict['clinic'] = tmp.astype(np.float32)
        common_idx_list.append(set(filtered_idx))
        for idx in filtered_idx:
            update_modality_combinations(idx, 'C')
        encoder_dict['clinic'] = PatchEmbeddings(feat_dim, args.num_patches, args.hidden_dim).to(args.device)
        input_dims['clinic'] = feat_dim
        
    if 'G' in args.modality or 'g' in args.modality:
        genomic_data, feat_dim = load_ehr_genomic_data(genomic_path)
        tmp = np.zeros((len(id_to_idx), feat_dim))
        filtered_idx = []
        for pid in genomic_data:
            tmp[id_to_idx[pid]] = genomic_data[pid]
            observed_idx_arr[id_to_idx[pid]][modality_dict['genomic']] = True
            filtered_idx.append(id_to_idx[pid])
        data_dict['genomic'] = tmp.astype(np.float32)
        common_idx_list.append(set(filtered_idx))
        for idx in filtered_idx:
            update_modality_combinations(idx, 'C')
        encoder_dict['genomic'] = PatchEmbeddings(feat_dim, args.num_patches, args.hidden_dim).to(args.device)
        input_dims['genomic'] = feat_dim
