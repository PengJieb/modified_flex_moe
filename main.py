...
from data import load_and_preprocess_data, create_loaders, load_and_preprocess_ehr_data
import warnings
    ...
    for batch_samples, batch_labels, batch_mcs, batch_observed in loader:
        ...
        if is_training:
            ...
            if outputs.shape[-1] > 2:
                ...
                all_probs.extend(torch.nn.functional.softmax(outputs, dim=1).detach().cpu().numpy()[:, 0])
...
def train_and_evaluate(args, seed, save_path=None):
    ...
    if args.data == 'adni':
        ...
        modality_dict = {'clinic': 0, 'genomic': 1}
        args.n_full_modalities = len(modality_dict)
        data_dict, encoder_dict, labels, train_ids, valid_ids, test_ids, n_labels, input_dims, transforms, masks, observed_idx_arr, full_modality_index = load_and_preprocess_ehr_data(args, modality_dict)
    ...
    if save_path is None:
        for epoch in trange(args.train_epochs):
            ...
            # print(val_labels, val_labels, val_probs)
